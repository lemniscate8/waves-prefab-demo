...
COPY . /opt/waves-prefab-demo

RUN \
  pip3 install -r /opt/waves-prefab-demo/docs/requirements.txt \
    && \
...
RUN \
  chgrp --recursive user /opt/waves-prefab-demo \
    && \
  chmod --recursive g+rwx /opt/waves-prefab-demo \
    && \
  echo "user granted permissions to /opt/waves-prefab-demo"

USER user

RUN \
  cd /opt/waves-prefab-demo \
    && \
